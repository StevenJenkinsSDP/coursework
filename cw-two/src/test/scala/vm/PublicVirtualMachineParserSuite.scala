...
class PublicVirtualMachineParserSuite extends FunSuite with ByteCodeValues {
  ...
  test("[8] vm parser should parse cw-two/programs/p01.vm a file into bytecode") {
    val code = vmp.parse("cw-two/programs/p01.vm")
  ...
  test("[8] vm parser should parse cw-two/programs/p03.vm a file into bytecode") {
    val code = vmp.parse("cw-two/programs/p03.vm")
  ...
  test("[4] vm parser should detect a program with invalid bytecode") {
    intercept[InvalidBytecodeException] {
      val code = vmp.parse("cw-two/programs/p04-bad-program.vm")
    }
  }
}
